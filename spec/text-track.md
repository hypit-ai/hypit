...
The current `packages/text-track` implementation has one fixed rectangular box, one plain string
...
<text:Track id="titles" space={film.space}>
  <text:Point id="hook" style={poster} timing={hook-time} placement={hook-point}>
    Nothing hidden in the Runtime.
  </text:Point>

  <text:Area id="explanation" style={word-pills} timing={body-time} placement={body-frame}>
    <text:P>Every <text:Span style={plain}>visible word</text:Span> stays authored.</text:P>
  </text:Area>
</text:Track>
```

This spelling is illustrative until shared temporal/spatial Surface syntax is fixed. Its important
properties are already final:

- the author imports and selects the Text package;
...
- the Text Surface, not SVS, interprets the Recipe vocabulary;
...
```text
Text Surface
  authored document + named Style + temporal binding + spatial binding
      │
      ▼
