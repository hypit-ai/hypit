...
## 10. Terminal Visual IR

`svml.visual-ir@1` carries the terminal Text forms:
...
renderer scripts to author packages. Text extrusion, material, light or camera semantics remain in
a separate component and enter Composition only through a typed alpha Surface. Unsupported rich
masks use the same path. These are video-terminal facts, never Core facts.
