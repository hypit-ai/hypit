...
// The VitePress original read `import.meta.env.BASE_URL`. Next serves `public/`
// from the site root, so assets live under `/demo/` with no build-time base.
export function publicAsset(relativePath: string) {
  return `/demo/${relativePath.replace(/^\/+/, "")}?v=${demoMediaVersion}`;
}
