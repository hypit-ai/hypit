import streetTranscript from "./street-transcript.json";
import goodBetterBestTranscript from "./gbb-transcript.json";
import { goodBetterBestMedia, streetMedia } from "./demo-media";
import { createDemoTimeline } from "./demo-timeline";

// Ported from the upstream `semantic-video-demos.ts`, unchanged apart from the
// transcript/media import paths.
...
export const semanticVideoDemos: Record<DemoId, SemanticVideoDemoConfig> = {
  street: {
    ...
    ] as DemoSelection[]).map((selection) => streetTimeline.mapRange(selection)),
    lines: streetLines,
  },
  "good-better-best": {
    id: "good-better-best",
    ...
    ] as DemoSelection[]).map((selection) => goodBetterBestTimeline.mapRange(selection)),
