...
Read this when turning useful visual references, voice identity, Script or a visual idea into living
speech, interaction, camera behavior or silent action. An image can establish who and where; generated
video decides what visibly and audibly happens over time.
...
| Script dialogue | the exact words and speaking turns for a visible performance |
...
Reference when supported, and the Segment's dialogue. Voice Design establishes who the person sounds
like; its short casting line does not need to repeat the finished Script. Independent speech is a
different A-roll construction, described in [Voice and performance](voice-and-performance.md).
...
playful skepticism, quiet concentration or another relationship the audience can recognize. Then
choose a few visible actions that communicate it. A restrained lean, open-palmed explanation, knowing
shrug or short glance can carry more than elaborate choreography. Vibe establishes the performance;
describe the decisive physical action when the story needs a handoff, reaction, entrance or exit.
...
Creator footage can benefit from pause-trim jump cuts. A podcast can cut with a speaker or toward a
meaningful reaction. A street interview can favor the guest and use the interviewer view for a
reaction. Stable framing, expressive acting and frequent edits are compatible choices.
...
Watch the result for engaged performance, correct speaking identity, legible interaction, intentional
camera behavior and the intended energy. Correct words alone do not establish that the scene works;
revise the visible or audible problem at the direction, reference, generation or media operation that
owns it.
