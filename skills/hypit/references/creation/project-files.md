...
```text
references/darby/
productions/darby-hypit/
productions/darby-hypit-short/
```

Each production links to `../../references/darby/` from its own Brief or Treatment and keeps its own
Sources and Runs.

The CLI finds the project from the nearest `package.json`; Source and Run files may live in any
...
- `PROGRESS.md` is a short photograph of the work now: the live question, next useful action, real
  blockers, and reusable Results. Established conclusions belong in their owning document instead.
