...
export type ScreenOverlayFragmentItem =
  ...
  | { readonly kind: "selection"; readonly specName: string; readonly sourceName: string }
  | { readonly kind: "moment"; readonly specName: string; readonly sourceName: string };
...
export function createScreenOverlayFragment(items: readonly ScreenOverlayFragmentItem[]) {
  ...
  const types = new Map<string, typeof screenOverlayTypes.itemSpec | typeof semanticTrackTypes.track | typeof narrativeTypes.selection | typeof narrativeTypes.moment>();
  ...
  items.forEach((item, index) => {
    ...
    operations.push({ id, producer: item.kind === "program" ? screenOverlayProducers.appendProgram : item.kind === "selection" ? screenOverlayProducers.appendSelection : screenOverlayProducers.appendMoment,
      inputs: { set: operation(current), header: input("header"), semantic: input("semantic"), spec: input(item.specName),
        ...(item.kind === "program" ? {} : { [item.kind]: input(item.sourceName) }) }, result: { kind: "output", name: "set" } });
    current = id;
  });
  operations.push(
