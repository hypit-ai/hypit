...
test("media identities and selection requests have package-owned semantic validators", () => {
  ...
  assert.ok(registry.resolve(mediaPipelineTypes.transformProgram));
  assert.ok(registry.resolve(mediaPipelineTypes.audioExtractionRequest));
  assert.ok(registry.resolve(mediaPipelineTypes.frameExtractionRequest));
});
