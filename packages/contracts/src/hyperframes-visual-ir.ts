 ...
 */
export const HYPERFRAMES_VISUAL_IR_V1 = "svml.hyperframes-visual-ir@1" as const;

/**
 * Closed CSS-shaped vocabulary for HyperFrames Visual IR v1.
 *
 ...
 */
export const HYPERFRAMES_VISUAL_STYLE_NAMES_V1 = [
...
export type HyperframesVisualStyleNameV1 = typeof HYPERFRAMES_VISUAL_STYLE_NAMES_V1[number];

const STYLE_NAMES = new Set<string>(HYPERFRAMES_VISUAL_STYLE_NAMES_V1);
const CROSS_TRACK_STYLES = new Set(["backdrop-filter", "mix-blend-mode"]);

export const HYPERFRAMES_VISUAL_STYLE_ENUM_VALUES_V1 = {
  "box-sizing": ["border-box", "content-box"],
  direction: ["ltr", "rtl"],
  display: ["block", "inline-block", "flex", "inline-flex", "grid", "inline-grid", "-webkit-box", "none"],
...
} as const satisfies Partial<Record<HyperframesVisualStyleNameV1, readonly string[]>>;

const ENUM_VALUES = new Map<string, ReadonlySet<string>>(
  Object.entries(HYPERFRAMES_VISUAL_STYLE_ENUM_VALUES_V1)
    .map(([name, values]) => [name, new Set<string>(values)]),
);

export function assertHyperframesVisualStyleV1(
  name: string,
  value: string | number,
  label: string,
): void {
  ...
  if (!STYLE_NAMES.has(name)) {
    throw new Error(`${label} style ${name} is outside ${HYPERFRAMES_VISUAL_IR_V1}.`);
  }
