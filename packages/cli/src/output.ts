...
function commandHelp(topic: string, colors: Palette): readonly string[] | undefined {
  ...
  const topics: Readonly<Record<string, readonly string[]>> = {
    ...
    "new-package": [
      colors.accent(colors.strong("hypit new-package")),
      colors.dim("Scaffold one component package: a Markup element and the code behind it."),
      "",
      "  hypit new-package <package-name> [--to <directory>] [--no-visual]",
      "",
      "  <package-name>             the full name, such as @hypit/local-notepad-list",
      "  --to <directory>           where to write it; defaults to packages/<name-after-the-scope>",
      "  --no-visual                the component draws nothing, so skip the still render and preview",
      "",
      "It writes the Manifest, Types, Surface, Producers, Activation and README, plus a",
      "still-render entry that draws one frame into preview/<Tag>.png through the local",
      "HyperFrames Runtime. Nothing is generated it cannot decide: the nominal Types, the",
      "Producers and the renderer are left with the comment saying where they go.",
      "The directory must be empty, and only a package under a workspace glob is linked",
      "by pnpm install and typechecked by pnpm check.",
    ],
    auth: [
...
export function writeCliHelp(io: CliIo, topic?: string): void {
  ...
  io.write([
    ...
    row("new-package <package-name>", "scaffold one component package"),
    "",
