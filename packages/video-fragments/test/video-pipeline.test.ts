...
import { captionImplementationDigest, captionProducers, temporalizeCaption } from "@svml/caption";
import {
  sealProgramSpace,
  sealSpeechBasis,
} from "@svml/contracts";
...
test("the Fragment-built speech pipeline resumes without repeating paid calls", async () => {
...
test("an Existing SpeechTake cuts generation while a visual substitute cuts the whole upstream", () => {
  ...
  const black: SpeechVisualTrack = {
    contract: "svml.speech-visual-track@1",
    basisDigest: digestOf("preview:black-basis"),
    narrativeDigest: narrative.semanticIndex.digest,
    programSpace: blackProgram,
    visualTrack: { clips: [] },
    segments: [{
      segmentId: "line",
      startSec: 0,
      endSec: 1,
      sourceArtifactDigest: digestOf("preview:black-artifact"),
    }],
  };
