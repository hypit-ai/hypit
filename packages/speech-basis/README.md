...
copy an upstream identity. Three ordinary deterministic Producers project it into `ProgramSpace`,
`VisualTrack` and `AudioTrack`. Core has no Projection primitive and never treats
...
`speechBasisComponent` enumerates all three Producer facets. Their identities are checked against the
