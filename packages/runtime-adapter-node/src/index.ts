...
export const nodeRuntimeHostAdapterAbi = "svml.node-runtime-host-adapter@1";
...
export type NodeRuntimeHost = {
  ...
  createRuntime(options?: { readonly implementationPackages?: LoadedNodePackageSet }): Promise<RuntimeHostExecution>;
...
export async function loadNodeRuntimeHost(
  ...
  const runtimePackageLock = resolve(profileRoot, nonEmpty(value.runtimePackageLock, "Runtime Profile runtimePackageLock"));
  ...
  const loaded = await loadNodePackageSelection(runtimePackageLock, {
    selected: [],
    logical: [{ abi: nodeRuntimeHostAdapterAbi, name: use }],
  }, options.packageRoot);
  const matches = loaded.packages.flatMap((item) => item.contribution.hostFacets ?? [])
