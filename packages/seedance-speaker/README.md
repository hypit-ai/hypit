...
same Surface renders the low-level `@svml/seedance` `SpeechProgram`; the Run Graph then reuses the
...
The Host package lock activates `@svml/prompt-kit` and low-level `@svml/seedance` independently. A
