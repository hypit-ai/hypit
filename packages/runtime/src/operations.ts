...
export function verifyOperationSnapshot(snapshot: OperationSnapshot): void {
  ...
  if (snapshot.status === "pending") assert(snapshot.checkpoint !== undefined && presence === 1, "pending Operation requires only a checkpoint");
...
export class MemoryOperationStore implements OperationStore {
  ...
  async compareAndSwap(
    ...
    const mutable = update.status === "pending"
      ? { status: "pending" as const, checkpoint: structuredClone(update.checkpoint) }
