...
export const decodeTextStyleSurface: StructuredSurfaceHandler = ({ element, resolveReference }) => {
  ...
  return { records: [{ id, type: textTrackTypes.style, value: { kind: "inline", value: style }, range: element.range }], components: [], fragments: [] };
};
...
export const decodeTextMotionSurface: StructuredSurfaceHandler = ({ element }) => {
  ...
  return { records: [{ id, type: textTrackTypes.motion, value: { kind: "inline", value: motion }, range: element.range }], components: [], fragments: [] };
};
...
function paragraph(element: StructuredElement, index: number, resolve: (path: string) => SurfaceResolvedReference | undefined): VisualTextDocument["paragraphs"][number] {
  ...
  for (const child of element.children) {
    ...
    const styleRef = child.attributes.style === undefined ? undefined : reference(child.attributes.style, `${child.name}.style`, textTrackTypes.style, resolve);
    inlines.push({
  ...
  const styleRef = element.attributes.style === undefined ? undefined : reference(element.attributes.style, `${element.name}.style`, textTrackTypes.style, resolve);
...
function createTrackFragment(id: string, items: readonly FragmentItem[]): GraphFragment {
  ...
  const operations: FragmentOperation[] = [{ id: "text:set:empty", producer: textTrackProducers.createSet, inputs: {}, result: { kind: "output", name: "set" } }];
  let current = "text:set:empty";
  for (const [index, item] of items.entries()) {
    const bind = `text:placement:${String(index + 1).padStart(4, "0")}`;
    operations.push({
      id: bind,
      producer: item.placementKind === "point" ? textTrackProducers.bindPoint : item.placementKind === "area" ? textTrackProducers.bindArea : textTrackProducers.bindPath,
    ...
    const common = {
      ...
      spec: input(item.specName), style: input(item.styleName), motion: input(item.motionName),
    };
    operations.push(item.binding === "program" ? {
      id: append, producer: textTrackProducers.appendProgram, inputs: common, result: { kind: "output", name: "set" },
    } : item.binding === "selection" ? {
      id: append, producer: textTrackProducers.appendSelection, inputs: { ...common, map: input("map"), selection: input(item.sourceName!) }, result: { kind: "output", name: "set" },
    } : {
      id: append, producer: textTrackProducers.appendMoment, inputs: { ...common, map: input("map"), moment: input(item.sourceName!) }, result: { kind: "output", name: "set" },
    });
    current = append;
  }
  operations.push(
  ...
  const inputEntries = [
    ...
    ...items.flatMap((item) => [
      ...
      { name: item.styleName, type: textTrackTypes.style },
      { name: item.motionName, type: textTrackTypes.motion },
  ...
  return sealGraphFragment({
    name: `@narratage/text-track/surface/${id}@1`, inputs, operations,
    exports: [
      {
        name: "program", type: textTrackTypes.program, root: operation("text:finalize"),
...
export const decodeTextTrackSurface: StructuredSurfaceHandler = ({ element, resolveReference }) => {
  ...
  const records: SurfaceRecordDraft[] = [{
    id: headerId, type: textTrackTypes.header,
    value: { kind: "inline", value: sealTextTrackHeader({ contract: "svml.text-track-header@1", id }) }, range: element.range,
  }];
  const defaultMotionId = `${id}.__still-motion`;
  records.push({ id: defaultMotionId, type: textTrackTypes.motion, value: { kind: "inline", value: stillTextMotion(defaultMotionId) }, range: element.range });
  ...
  for (const child of element.children) {
    ...
    allowed(child, [
      "id", "placement", "style", "motion", "during", "at", "for", "start", "end", "selection", "moment", "occurrences",
    ], ["id", "placement", "style"]);
    ...
    const style = reference(child.attributes.style, `${child.name}.style`, textTrackTypes.style, resolveReference);
    const motionRef = child.attributes.motion === undefined
      ? ({ kind: "record", id: defaultMotionId } as const)
      : reference(child.attributes.motion, `${child.name}.motion`, textTrackTypes.motion, resolveReference).ref;
  ...
  const fragmentItems: FragmentItem[] = items.map(({ suffix, binding, sourceName, placementKind, geometryName, specName, styleName, motionName }) => ({
    suffix, binding, placementKind, geometryName, specName, styleName, motionName, ...(sourceName === undefined ? {} : { sourceName }),
  }));
...
function createMaskFragment(id: string): GraphFragment {
  const inputs = [
    ...
    { name: "spec", type: textTrackTypes.maskSpec },
  ];
  return sealGraphFragment({
    name: `@narratage/text-track/mask-surface/${id}@1`,
    inputs,
    operations: [{
      id: "text-mask:render", producer: textTrackProducers.renderMask,
...
export const decodeTextMaskSurface: StructuredSurfaceHandler = ({ element, resolveReference }) => {
  allowed(element, ["id", "space", "text", "material", "mode", "fit"], ["id", "space", "text", "material"]);
  empty(element);
  const id = text(element, "id");
  const space = reference(element.attributes.space, `${element.name}.space`, programSpaceTypes.programSpace, resolveReference);
  const program = reference(element.attributes.text, `${element.name}.text`, textTrackTypes.program, resolveReference);
  ...
  return {
    records: [{ id: specId, type: textTrackTypes.maskSpec, value: { kind: "inline", value: spec }, range: element.range }],
    components: [{
      id, fragment: fragment.id,
