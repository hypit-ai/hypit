...
test(".svrun compiles one multi-export Fragment instance before execution", async () => {
  const document = parseRunDocument("preview.svrun", `<svrun version="1" source="./main.svml" targets="preview">
  ...
  assert.equal(run.graphs.length, 1);
  assert.equal(run.graphs[0]!.operations.length, 1);
  ...
  const request = sealBuildRequest({
    ...
    targets: run.targets.map((item) => ({ output: item.name, accepts: item.accepts })),
    satisfactions: run.satisfactions,
  });
...
test("separate Fragment declarations remain separate executions", async () => {
  ...
  assert.equal(run.graphs[0]!.operations.length, 2);
  assert.equal(new Set(run.graphs[0]!.operations.map((item) => item.id)).size, 2);
});

test("a Provided Value is an ordinary zero-input Candidate selected by Satisfaction", async () => {
  const document = parseRunDocument("fixed.svrun", `<svrun version="1" source="./main.svml" targets="preview">
  ...
  const realized = resolveRealization(compilation.program, compilation.elaboration.graph, run.graphs);
...
test("Run imports are a prologue and Runtime settings are not language elements", () => {
  assert.throws(
    () => parseRunDocument("bad.svrun", `<svrun version="1" source="main.svml" targets="x">
