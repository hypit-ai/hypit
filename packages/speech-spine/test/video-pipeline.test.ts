...
import { compositionTypes, sealVisualTrack } from "@narratage/composition";
...
test("the Speech Spine pipeline resumes without repeating paid calls", async () => {
  ...
  host.registerProducer(
    speechBasisProducers.projectAudio,
    projectSpeechAudioImplementationDigest,
    ({ inputs }) => {
  ...
  host.registerProducer(
    speechBasisProducers.projectVisual,
    projectSpeechVisualImplementationDigest,
    ({ inputs }) => {
  ...
  registerProducerFacets(host, speechAlignmentComponent.producers);
  ...
  const evidenceInput = mapDerivation.inputs.find((input) => {
    ...
    return record?.type.name === speechEvidenceTypes.alignedTranscript.name
      && record.type.module.name === speechEvidenceTypes.alignedTranscript.module.name
      && record.type.module.version === speechEvidenceTypes.alignedTranscript.module.version;
  });
...
test("Targets prune official Fragments while shared SpeechBasis generation stays singular", () => {
  ...
  assert.equal(producerCount(map, speechBasisProducers.projectVisual), 0);
  ...
  assert.equal(producerCount(bothTakeProjections, speechBasisProducers.projectAudio), 1);
  assert.equal(producerCount(bothTakeProjections, speechBasisProducers.projectVisual), 1);
  ...
  assert.equal(producerCount(bothEvidenceExports, speechAlignmentProducers.locate), 1);
});

test("an Existing SpeechBasis cuts generation while a visual substitute cuts the whole upstream", async () => {
