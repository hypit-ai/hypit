...
export type AlignedTranscriptSegment = { readonly sourceSegmentId: string; readonly startSec: number; readonly endSec: number;
...
export type AlignedTranscriptEvidence = { readonly contract: "svml.aligned-transcript-evidence@1"; readonly durationSec: number; readonly segments: readonly AlignedTranscriptSegment[] };
...
export const alignedTranscriptEvidenceFields = {
  contract: { schema: { kind: "literal", value: "svml.aligned-transcript-evidence@1" } }, durationSec: { schema: number },
  segments: { schema: { kind: "array", minItems: 1, items: object({ sourceSegmentId: { schema: string }, startSec: { schema: number }, endSec: { schema: number },
