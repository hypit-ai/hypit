...
- `runtime-component` constructs a Scheduler, Store or Credential Component package.

The two kinds have separate Host ABIs, so an Endpoint and Runtime Component may intentionally share
...
Runtime Component adapters retain a separate pure `validate()` gate because constructing SQLite or a
