...
function registry(options: { readonly omitMeasureFragment?: boolean } = {}): TextSurfaceRegistry {
  const values = new TextSurfaceRegistry();
...
test("Text rejects a component whose Surface omits its Fragment definition", async () => {
  await assert.rejects(
    ...
    (error: unknown) => error instanceof TextFrontendError
      && error.code === "TEXT_COMPONENT_FRAGMENT_MISSING",
  );
});
...
test("Text rejects duplicate component identities before Author linking", async () => {
  await assert.rejects(
    ...
    (error: unknown) => error instanceof TextFrontendError
      && error.code === "TEXT_COMPONENT_DUPLICATE",
  );
});
