...
values whose duration or media inputs arrive through later graph edges. `omit` means another
Producer owns that port; `defer` means a graph assembly may attach the port before exact
finalization. Neither option weakens the request accepted by a Provider.

## Graph-native request assembly

A generated reference is not metadata copied through an authored value. It remains an ordinary
Blob edge all the way into the exact-model package:

```text
scalar RequestDraft ───────────────┐
                                   ├─ bind(referenceImage) ─┐
generated image Blob ──────────────┘                         │
                                                             ├─ finalize ─> GenerationRequest ─> Need
audio Binding + generated audio Blob ─ bind(referenceAudio) ─┘
```

Every exact-model module generated by `@narratage/model-kit` owns four related things:

- a model-specific Request Draft Type whose media ports may still be absent;
- one model-specific Binding Type and pure bind Producer per media port;
- a pure finalize Producer that applies the complete Port Table;
- the existing exact request Type and Need Producer.

A Binding carries only the declared role and any port item fields. The Blob is a separate Producer
input, so the Author Graph records the real dependency rather than hiding it inside a prebuilt
Record. Intermediate drafts may temporarily violate a cross-port rule such as “audio requires a
visual reference”; finalization applies every cardinality, MIME and combination rule before a
Provider can see the request.

This is also the Candidate boundary. If a Run Graph supplies a zero-input image Candidate, normal
demand planning cuts the replaced image branch. If a generated image remains selected, that branch
runs exactly once even when several downstream requests consume it. There is no model cache guess,
source-digest convention or special Pin mechanism.

Seedance video and speech references, MiMo voice-clone samples and the GPT Image clean composite use
this same assembly path. Prompt construction remains an independent author-compilation concern; it
supplies a scalar `prompt` value and has no authority over the media edges.
