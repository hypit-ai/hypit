...
| 3. Freeze format and craft | Select the matching playbook and load its required craft documents. For original authoring, establish recurring person, product, voice, location, wardrobe, lighting and prop anchors before writing generation prompts, then reuse them across takes. | `playbooks/index.md`, `formats/*.md`, `craft/visual-continuity.md`, `craft/persona-and-audio.md`, `craft/generated-dependencies.md` | The program gets a timing model, shot grammar, caption rules and explicit cross-shot identity/scene continuity even without a reference video. |
