...
| 包名 | `@svml/` scope | `@svml/speech-align` |
| Provider 包 | `provider-` 前缀 | `@svml/provider-kie` |
...
- 跨包导入使用 `@svml/*`，绝不跨包边界使用相对路径。
- 禁止循环依赖（由 `tools/package-boundaries.test.mjs` 强制执行）。

## TypeScript 配置

`tsconfig.v2.json` 继承 `tsconfig.json`，并为所有 `@svml/*` 包添加 `paths` 映射。
...
新增一个包时，要把它的路径映射加入 `tsconfig.v2.json`：
...
- 所有持久化数据使用 `@2` wire 格式版本。
- wire 类型定义在 `@svml/protocol` 中，且不可变。
