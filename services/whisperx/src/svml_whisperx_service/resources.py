...
MARKER_NAME = ".svml-resource.json"


def default_nltk_data_root() -> Path:
    return Path.home() / ".cache" / "svml" / "whisperx" / "nltk_data"
...
def assert_punkt_tab(root: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "locked NLTK punkt_tab data is unavailable; run `svml-whisperx-prepare`"
        ) from error
...
def prepare_punkt_tab(root: Path, payload: bytes | None = None) -> Path:
    ...
    target.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="svml-punkt-tab-", dir=target.parent) as temporary:
